...
## [2024-03-21] - Diagonal Streak Animation Fix
### Fixed
- Fixed diagonal streak animations in 2-players-plus mode to match the working implementation from 2-players mode
- Corrected streak dimensions (850px width) for proper diagonal coverage
- Fixed positioning with proper margins and transform origin
- Improved animation timing and transitions
- Added blur effects and smooth gradients for better visual appearance
- Made all streak animations (horizontal, vertical, diagonal) consistent in behavior and timing

### Technical Details
- Updated streak element base properties in AdvancedGameBoard.module.css
- Corrected transform and animation properties for diagonal streaks
- Simplified animation keyframes for more predictable behavior
- Changed animation duration from 2s to 1.5s for snappier feedback
- Removed unnecessary scale transforms that were causing visual issues

## [2024-01-09]

### Changed
- Reduced gameboard margins in 2-player mode for a more compact layout
  - Top margin reduced from 2rem to 1rem
  - Bottom margin reduced from 1rem to 0.5rem
  - Updated in TestGameBoard.module.css
